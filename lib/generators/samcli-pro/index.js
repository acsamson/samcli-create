...
class AntDesignProGenerator extends BasicGenerator {
  ...
  async writing() {
    ...
    // 重新生成一个package.json
    if (pkg['samcli-creat']) {
      const { ignoreScript = [], ignoreDependencies = [] } = pkg['samcli-creat'];
      ...
      // remove samcli-creat config
      delete projectPkg['samcli-creat'];
      fs.writeFileSync(
    ...
    // 删掉一些没有用的文件
    if (pkg['samcli-creat'] && pkg['samcli-creat'].ignore) {
      log('清理中...');
      const ignoreFiles = pkg['samcli-creat'].ignore;
