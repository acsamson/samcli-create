...
const getGithubUrl = async () => {
    ...
    if (fastGithub === 'gitee.com' || fastGithub === 'github.com.cnpmjs.org') {
        return 'https://gitee.com/acsamson/samcli-pro';
    }
...
class AntDesignProGenerator extends BasicGenerator {
        ...
        if (this.opts.args.language) {
            ...
            const prompts = [
                ...
                // 第2步选择模板类型
                {
                    ...
                    type: 'list',
                    message: '💁‍♂️ 想使用哪种脚手架?',
                    choices: ['前端中台:gitee(recommend)', '前端中台:github', '全栈(微前端中台 + node服务端):github'],
                    default: '前端中台:gitee(recommend)',
                },
            ];
    ...
    async writing() {
        ...
        if (allBlocks === '前端中台:gitee') {
            log(`🏃‍♂️ 准备下载前端中台:gitee...`);
        } else if (allBlocks === '前端中台:github') {
            log(`🏃‍♂️ 准备下载前端中台:github...`);
            gitArgs.push('--branch', 'fepro');
        } else if (allBlocks === '全栈(微前端中台 + node服务端):github') {
            log(`🏃‍♂️ 全栈(微前端中台 + node服务端):github...`);
            gitArgs.push('--branch', 'fullstack');
        }
        ...
        log(`👌 克隆成功`);
        if (allBlocks === '前端中台:github' || allBlocks === '前端中台:gitee(recommend)') {
            ...
            if (pkg['samcli-create']) {
                const {
                    ignoreScript = [], ignoreDependencies = []
                } = pkg['samcli-create'];
