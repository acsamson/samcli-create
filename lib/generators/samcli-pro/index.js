...
class AntDesignProGenerator extends BasicGenerator {
  ...
  async writing() {
    ...
    // 重新生成一个package.json
    if (pkg['create-samcli']) {
      const { ignoreScript = [], ignoreDependencies = [] } = pkg['create-samcli'];
      ...
      // remove create-samcli config
      delete projectPkg['create-samcli'];
      fs.writeFileSync(
    ...
    // 删掉一些没有用的文件
    if (pkg['create-samcli'] && pkg['create-samcli'].ignore) {
      log('清理中...');
      const ignoreFiles = pkg['create-samcli'].ignore;
